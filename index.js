...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  if (arr.length > maxLength) {
    console.warn(
      'Увага! Довжина масиву перевищує максимально допустиму!',
    )
  } else {
    console.log(
      'Довжина масиву не перевищує максимально допустиму.',
    )
  }
...
function compareArrays(arr1, arr2) {
  ...
  const jsonArr = JSON.stringify(sortArr)
  const jsonArr2 = JSON.stringify(sortArr2)
  let comparArr = jsonArr === jsonArr2
  console.assert(comparArr, ': Масиви не однакові! ')
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      arrS[j] = Math.abs(Math.ceil(Math.random() * 10 - 1))
    }
    matrix[i] = arrS
  }
...
function countVowelsAndConsonants(word) {
  ...
  for (let item of word) {
    ...
    if (literals.includes(lowerItem)) {
